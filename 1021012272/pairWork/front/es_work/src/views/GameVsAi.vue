...
import getSelectApi from "@/api/socre/getSelect";
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      if (round.value == 3) {
        ...
        let arr1 = [];
        let arr2 = [];
        for (let w = 0; w < selectArea1.value.length; w++) {
          arr1.push(selectArea1.value[w] + 1);
        }
        for (let w = 0; w < player1.value.length; w++) {
          arr2.push(player1.value[w] + 1);
        }
        arr1.sort((a, b) => a - b);
        arr2.sort((a, b) => a - b);
        idForm.id1 = arr1.join("");
        ...
        arr1 = [];
        arr2 = [];
        for (let w = 0; w < selectArea2.value.length; w++) {
          arr1.push(selectArea2.value[w] + 1);
        }
        for (let w = 0; w < player2.value.length; w++) {
          arr2.push(player2.value[w] + 1);
        }
        arr1.sort((a, b) => a - b);
        arr2.sort((a, b) => a - b);
        idForm.id21 = arr1.join("");
        idForm.id22 = arr2.join("");
        setTimeout(() => {
          getSelectApi(idForm).then((res) => {
            const data = res.data.data;
            console.log(data);
            for (let i = 0; i < data.length; i++) {
              for (let j = 0; j < player1.value.length; j++) {
                if (player1.value[j] == data[i] - 1) {
                  selectArea1.value.push(data[i] - 1);
                  player1.value.splice(j, 1);
                  break;
                }
              }
            }
            if (res.data.isRage) {
              multiple.value += res.data.rage;
              ElMessage({
                message: "AI已锁定骰子并选择加" + res.data.rage + "倍",
                type: "success",
              });
            } else {
              ElMessage({
