<template>
  <div class="home" v-cloak>
    ...
    <div class="player1_area">
      ...
      <div class="bg"></div>
      <div class="choose" v-if="isSure1 != 0" @click="readyForGame(1)">
        {{ buttonInfo[isSure1 - 1] }}
      </div>
      <div
        class="lock"
        :class="{ beforeStart: ready1 == 2 }"
        @click="lockDice(1)"
      ></div>
      <div class="status">
        {{ statusList[ready1] }}
      ...
      <div
        class="plus"
        :class="{ beforeStart: isPlus1 == 0 }"
        @click="chooseIsPlus(1)"
      ></div>
      <div class="money"></div>
      <div class="point">${{ money1 }}</div>
      <div class="jetton1" v-if="isPlus1 == 1">
        <div class="number" @click="choosePlus(1, 1)">一倍</div>
        <div class="number" @click="choosePlus(1, 2)">两倍</div>
        <div class="number" @click="choosePlus(1, 3)">三倍</div>
        <div class="number" @click="choosePlus(1, 4)">取消</div>
      </div>
      <div class="dice-box">
    ...
    <div class="dice">
      <div class="dice-box">
        <div
          class="box"
          :class="{ beforeStart: ready1 == 2 }"
          v-for="(item, index) in player1"
          :key="index"
          @click="selectDice(1, index)"
        >
          <div v-for="(item2, index2) in imgUrl" :key="index2">
      ...
      <div class="leftInfo">
        ...
        <div class="music">
          ...
          <!--音乐结束-->
          <div @click="stopMusic()" v-if="stopBackgroundMusic">
            <img src="@/assets/sound.png" />
          </div>
          <div @click="playMusic()" v-if="!stopBackgroundMusic">
    ...
    <div class="player2_area">
      ...
      <div class="jetton2" v-if="isPlus2 == 1">
        <div class="number" @click="choosePlus(2, 1)">一倍</div>
        <div class="number" @click="choosePlus(2, 2)">两倍</div>
        <div class="number" @click="choosePlus(2, 3)">三倍</div>
        <div class="number" @click="choosePlus(2, 4)">取消</div>
      </div>
    </div>
  </div>
  ...
  >
    <div class="endMsg">
      ...
      <div class="scorexx1">{{ score1 }}</div>
      <div class="scorexx">{{ score2 }}</div>
      <div class="moneyxx">{{ moneyxx }}</div>
      <div class="moneyxx1">{{ moneyxx }}</div>
      <div v-if="score1 > score2" class="player1b"></div>
      <div v-if="score1 < score2" class="player1a"></div>
      <div v-if="score1 > score2" class="player2a"></div>
      <div v-if="score1 < score2" class="player2b"></div>
...
const buttonInfo = ["准备", "结束回合"];
const money1 = ref(10000);
const money2 = ref(10000);
...
function choosePlus(option, number) {
  if (number == 4) {
    if (option == 1) {
      isPlus1.value = 0;
    } else {
...
function handlestart() {
  ...
  setTimeout(() => {
    ...
    //随机1-6点数
    for (var i = 0; i < player1.value.length; i++) {
      player1.value[i] = Math.round(Math.random() * 5);
    }
      ...
      if (round.value == 3) {
        ...
        setTimeout(() => {
          getScoreApi(idForm).then((res) => {
            ...
            moneyxx.value = Math.abs(score * multiple.value);
            if (money1.value + score * multiple.value < 0) {
              money1.value = 0;
              money2.value += money1.value;
            } else if (money2.value - score * multiple.value < 0) {
              money2.value = 0;
              money1.value += money2.value;
            } else {
              money1.value += score * multiple.value;
              money2.value -= score * multiple.value;
            }
...
.result {
  width: 98vh;
  height: 70vh;
  background-image: url("https://bucketofpicture.oss-cn-hangzhou.aliyuncs.com/result.png");
  background-size: cover;
  background-size: 100% 100%;
}
